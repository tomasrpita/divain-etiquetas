...
if os.name == "nt":
    ...
    api_prodruction_order = (
        current_app.config["BASE_URL"] + current_app.config["PRODUCTION_ORDER_URL"]
    )
else:
    # mac
    printers = fake_printer_job
    on_production = False
    api_prodruction_order = (
        "http://127.0.0.1:8000/" + current_app.config["PRODUCTION_ORDER_URL"]
    )
...
def home():
    ...
    if request.method == "POST":
        ...
        if error or not labels_info:
            ...
            flash(
                f"Error tratando de obtener los datos de las etiquetas: {error_message}",
                "danger",
            )
            abort(404)
