					...
					if labels_info["ingredient_lines"]["start"] <= line_number <= labels_info["ingredient_lines"]["end"]:
						...
						if index_exists(lista_ingredientes, index):
							s = s.replace(
								...
								(
									line.replace(
										b"####################", bytes(lista_ingredientes[index], "utf-8")
									)
								),
							)
						else:
						...
						s = s.replace(line, (line.replace(b"XXXXXX", bytes(self.lote, "utf-8"))))

					elif line_number == labels_info["sku_box_line"]:
						# SKU box
						s = s.replace(line, (line.replace(b"DIVAIN-ZZZ", bytes(self.sku, "utf-8"))))

					elif line_number == labels_info["barcode_box_line"]:
						# Codebar box
						ean_select = self.ean_botes[:-1] + "!100" + self.ean_botes[-1:]
						s = s.replace(line, (line.replace(b"123456789012!1003", bytes(ean_select, "utf-8"))))

					elif line_number == labels_info["ean_box_line"]:
						# Ean box
						s = s.replace(line, (line.replace(b"1234567890123", bytes(self.ean_botes, "utf-8"))))

					elif line_number == labels_info["lote_box_line"]:
						# Lote box
						s = s.replace(line, (line.replace(b"xxxxxxxxxx", bytes(self.lote, "utf-8"))))

					elif line_number == labels_info["copies_number_line"]:
						s = s.replace(
							line, (line.replace(b"1,1", bytes(f"{self.copies_mumber},1", "utf-8")))
						)
